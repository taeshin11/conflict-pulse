...
export default async function CountriesAtWarPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <main className="max-w-7xl mx-auto px-4 py-8 flex-1">
        ...
        <div className="mb-6">
          <h1 className="text-3xl font-extrabold text-slate-900 mb-2">Countries at War Right Now (2026)</h1>
          <p className="text-slate-600">
            A comprehensive list of all {sorted.length} countries currently experiencing armed conflict, war, or active military operations.
          </p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {sorted.map(c => <CountryCard key={c.id} country={c} />)}
        </div>
      </main>
